...
common.resetConsole = function (x, y, w, h) {
  ...
  //     console.setTitle("调试窗口", "#ff11ee00", 30);
 ...
 */
common.updateDeviceStatus = function (status) {
  status = status || 1
...
// 停止其他脚本的执行
common.stopOtherScript = function () {
    let current_e_id = engines.myEngine().id;
    let stopCount = 0;
    engines.all().filter((e) => {
        if (current_e_id !== e.id) {
            e.forceStop();
            stopCount++;
        }
    });

    stopCount > 0 && toast('已停止' + stopCount + '个运行的脚本');
    floaty.closeAll();
}
// 请求悬浮窗权限
common.requestFloatWindowPermission = function() {
    var sdkInt = android.os.Build.VERSION.SDK_INT;
    if (sdkInt >= 23) {
        try {
            var buildVersion = android.provider.Settings.class.getDeclaredField("CAN_DRAW_OVERLAYS");
            buildVersion.setAccessible(true);
            var settingsCanDraw = buildVersion.getBoolean(android.provider.Settings.class);
            if (!settingsCanDraw) {
            ...
            console.error("请求悬浮窗权限失败：", e);
            return false;
        }
    }
    return true;
}
common.getSelectRadioText = function(radioGroup) {
  let count = radioGroup.getChildCount();
  for (let i = 0; i < count; i++) {
      let rb = radioGroup.getChildAt(i);
      if (rb.isChecked()) {
          return rb.text;
      }
  }
